...
const CourseActions = ({ disabled, courseId, isPublished, setIsPublished }) => {
    ...
    const onDelete = async () => {
        try {
            setIsLoading(true);
            await fetch(`${VITE_APP_BACKEND_URL}/instructor/courses/${courseId}`, {
                method: 'DELETE',
                headers: {
    ...
    const onClick = async () => {
        try {
            setIsLoading(true);
            const publishToast = toast.loading(`${isPublished ? 'Unpublishing' : 'Publishing'} Course...`);
            const response = await fetch(`${VITE_APP_BACKEND_URL}/instructor/courses/${courseId}/${isPublished ? 'unpublish' : 'publish'}`, {
                method: 'PATCH',
                headers: {
                    authorization: `Bearer ${localStorage.getItem('token')}`
                }
