...
const Module = () => {
    ...
    const fetchModule = async () => {
        const response = await fetch(`http://localhost:3000/instructor/courses/${courseId}/modules/${moduleId}`, {
            method: 'GET',
            headers: {
    ...
    const moduleOnSubmit = async (values) => {
        try {
            setIsEditing(false);
            await fetch(`http://localhost:3000/instructor/courses/${courseId}/modules/${moduleId}`, {
                method: 'PATCH',
                headers: {
                    authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    const articleOnSubmit = async (values) => {
        try {
            setIsCreating(true);
            await fetch(`http://localhost:3000/instructor/courses/${courseId}/modules/${moduleId}/articles`, {
                method: 'POST',
                headers: {
                    authorization: `Bearer ${localStorage.getItem('token')}`,
                    'content-type': 'application/json'
